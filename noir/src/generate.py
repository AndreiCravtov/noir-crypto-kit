...
const_nr += "global ADD: [Field; 2*P - 1] = ["
add_table = [0]*(2*P - 1)
for a in range(0, P):
     for b in range(0, P):
          add_table[a + b] = (a + b) % P
const_nr += ", ".join([str(i) for i in add_table])
const_nr += "];\n\n"
...
const_nr += "global EXPONENTIATE: [Field; N*P] = [\n"
for s in range(0, P):
    #const_nr += "["
    const_nr += ", ".join([str((s ** i) % P) for i in range(0, N)]) + ",\n"
    #const_nr += "],\n"
const_nr += "];\n\n"

for m in range(0, M):
    const_nr += f"global MULTIPLIER_POLYNOMIAL_{m}: [Field; N] = ["
    const_nr += ", ".join([str(i) for i in MULTIPLIER_POLYNOMIAL_COEFFICIENTS[m]])
    const_nr += "];\n\n"
